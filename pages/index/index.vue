<template>
	<view class="container vs-flex-item vs-column">
		<!-- 导航栏 S -->
		<my-nav-bar :address="userInfo.shipAddress.position_name"
			:style="{top:old.scrollTop>10?'-37px':0}"></my-nav-bar>
		<!-- 导航栏 S -->

		<!-- 主体内容部分 S -->
		<view :style="{marginTop:old.scrollTop>10?'60px':'97px'}" class="content-body flex flex-sub">
			<!-- 获取到城市时显示 S -->
			<view v-if="userInfo.shipAddress.position_name" class="vs-column vs-space-center">
				...
				<view class="nav vs-column">
					<view class="bg-white vs-flex-item vs-column">
						<view class="grid margin-bottom text-center" :class="'col-5'">
							<navigator url="/pages/categoryList/categoryList"
								class="padding-xs flex vs-column vs-align-center" :style="{width:'150rpx'}"
								v-for="(item,index) in navList" :key="index">
								<image :src="item.image_hash,'w_90,h_90'|imgUrlFilter" mode="aspectFill"
									class="nav-img"></image>
								<text class="nav-title font-20 color-black6">{{item.name}}</text>
							</navigator>
						</view>
					</view>
				</view>
				...
				<!-- 广告区 S -->
				<navigator url="/pages/combo/combo" class="banner flex vs-row vs-space-between">
				...
				<view class="content-list flex-direction">
					...
					<store-list :top="navBarHeight" :showGotop="false" :nativeNav="false" :navTapFn="scrollToStoreList"
						:storeListData="storeListData" :hasNext="hasNext" :loading="storeListDataLoading"
						class="store-list"></store-list>
		...
		<!-- 回到顶部模块 S -->
		<gotop :gotopFn="goTop" :targetTop="300" :scrollTop="old.scrollTop"></gotop>
		<!-- 回到顶部模块 E -->
		...
		methods: {
			...
			scrollToStoreList() {
				...
				if (this.$utils.getElementInfo('.store-list').top > this.navBarHeight) {
					uni.pageScrollTo({
						duration: 0,
						scrollTop: this.$utils.getElementInfo('.store-list').top - this.$utils.getElementInfo(
							'.container').top - this.navBarHeight
					})
				}
				// #endif
			},
